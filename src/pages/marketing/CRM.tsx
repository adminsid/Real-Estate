...
import {
  ...
  X, Download, UserCheck, ListPlus, ArrowRight, AlertTriangle, ArrowUpRight, Upload
} from 'lucide-react'
  ...
  const handleImportCSV = async (e: React.FormEvent) => {
    ...
    try {
      ...
      for (const row of rows) {
        ...
        const firstName = getVal(['firstname', 'first', 'name']) || ''
        const lastName = getVal(['lastname', 'last']) || ''
        const email = getVal(['email', 'mail']) || null
        const phone = getVal(['phone', 'tel', 'mobile']) || null
        const type = getVal(['type', 'contacttype']) || 'buyer'
        const status = getVal(['status']) || 'prospect'
        const source = getVal(['source', 'leadsource']) || 'CSV Import'
      ...
      setImportSummary(`Successfully imported ${successCount} contacts. (${failCount} failed)`)
      loadContacts()
      loadStats()
      setCsvFile(null)
    } catch (err) {
      alert('Failed to parse or import CSV. Please ensure formatting matches: first_name, last_name, email, phone, etc.')
    } finally {
      setIsImporting(false)
    }
  }
  ...
  return (
    <Layout title="CRM — Lead & Contact Operating System">
      ...
      <div className="flex flex-wrap items-center gap-3 mb-4">
        ...
        <button onClick={() => setShowImportModal(true)} className="rounded-xl border border-gray-200 px-3 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50 inline-flex items-center gap-1.5">
      ...
      <div className="flex gap-4">
        ...
        {/* Productivity Quick Preview & Action Panel */}
        <aside className="hidden xl:block w-80 rounded-2xl border border-gray-200 bg-white shadow-sm h-fit sticky top-24 overflow-hidden">
          {previewContact ? (
            <div className="p-4 space-y-3 font-sans text-xs">
              <div className="flex items-start justify-between gap-2 border-b border-gray-100 pb-3">
                <div>
                  <p className="font-extrabold text-gray-900 text-sm">{previewContact.first_name} {previewContact.last_name}</p>
                  <span className="inline-block mt-0.5 px-2 py-0.5 rounded-full text-[10px] font-bold bg-blue-50 text-blue-800 border border-blue-200 capitalize">
                    {typeLabels[previewContact.type] ?? previewContact.type}
                  </span>
                </div>
                <button onClick={() => setPreviewContact(null)} className="rounded-lg p-1 text-gray-400 hover:bg-gray-100">
                  <X className="h-4 w-4" />
                </button>
              </div>

              {/* 1-Click Productivity Quick Action Buttons */}
              <div className="grid grid-cols-2 gap-2 pt-1">
                {previewContact.phone ? (
                  <a
                    href={`tel:${previewContact.phone}`}
                    className="flex items-center justify-center gap-1.5 py-2 px-2 rounded-xl bg-emerald-50 hover:bg-emerald-100 text-emerald-800 font-bold border border-emerald-200 text-center"
                  >
                    📞 Call Lead
                  </a>
                ) : (
                  <button disabled className="flex items-center justify-center gap-1.5 py-2 px-2 rounded-xl bg-gray-100 text-gray-400 font-semibold cursor-not-allowed">
                    📞 No Phone
                  </button>
                )}
                ...
                {previewContact.email ? (
                  <a
                    ...
                    className="flex items-center justify-center gap-1.5 py-2 px-2 rounded-xl bg-blue-50 hover:bg-blue-100 text-blue-800 font-bold border border-blue-200 text-center"
                  >
                    ✉️ Email
                  </a>
                ) : (
              ...
              {/* Details List */}
              <div className="space-y-1.5 bg-gray-50/80 p-3 rounded-xl border border-gray-200 text-[11px]">
                {previewContact.email && <p className="text-gray-700 truncate"><strong>Email:</strong> {previewContact.email}</p>}
                {previewContact.phone && <p className="text-gray-700"><strong>Phone:</strong> {previewContact.phone}</p>}
                {previewContact.source && <p className="text-gray-700"><strong>Source:</strong> {previewContact.source}</p>}
                {previewContact.next_follow_up_date && (
                  <p className="font-bold text-amber-800 bg-amber-50 p-1.5 rounded border border-amber-200 mt-1">
                    📅 Next Follow-up: {previewContact.next_follow_up_date}
                  </p>
                )}
              </div>
              ...
              <button
                onClick={() => {
                  const info = `${previewContact.first_name} ${previewContact.last_name} | Tel: ${previewContact.phone || 'N/A'} | Email: ${previewContact.email || 'N/A'}`
                  navigator.clipboard.writeText(info)
                  alert('Contact info copied to clipboard!')
                }}
                className="w-full py-1.5 px-3 rounded-xl border border-gray-300 text-gray-700 font-bold hover:bg-gray-100 flex items-center justify-center gap-1.5"
              >
                📋 Copy Contact Summary
              </button>

              <button
                onClick={() => navigate(`/marketing/crm/${previewContact.id}`)}
                className="w-full py-2 px-3 rounded-xl bg-slate-900 hover:bg-slate-800 text-white font-extrabold flex items-center justify-center gap-1.5 shadow-sm"
              >
                Full Details & History <ArrowRight className="h-3.5 w-3.5" />
              </button>
            </div>
          ) : (
            <div className="p-6 text-center text-xs text-gray-400">
              Select a contact from the list to preview quick productivity actions & full details.
            </div>
          )}
        </aside>
      </div>
          ...
          <div className="bg-white rounded-3xl p-6 w-full max-w-md shadow-2xl space-y-4">
            <div className="flex justify-between items-center border-b pb-2.5">
              <h3 className="text-base font-bold text-gray-900 flex items-center gap-1.5">
                ...
                CRM CSV Contact Import
              </h3>
              <button onClick={() => { setShowImportModal(false); setImportSummary(null); }} className="text-gray-400 hover:text-gray-600">✕</button>
            </div>

            {importSummary ? (
              <div className="p-4 bg-emerald-50 border border-emerald-100 rounded-2xl text-center space-y-2">
                <p className="text-xs text-emerald-800 font-bold">{importSummary}</p>
                <button
              ...
              <form onSubmit={handleImportCSV} className="space-y-4">
                <div className="p-3 bg-blue-50 border border-blue-100 rounded-2xl text-xs text-blue-800 space-y-1">
                  <p className="font-semibold">Supported Headers:</p>
                  <p className="text-blue-700">
                    Your CSV should include a header row containing fields like: <strong>first_name</strong>, <strong>last_name</strong>, <strong>email</strong>, <strong>phone</strong>, <strong>type</strong>, and <strong>notes</strong>.
                  </p>
                </div>

                <div className="border-2 border-dashed border-gray-200 rounded-2xl p-6 text-center hover:border-brand-navy transition-colors bg-gray-50/50">
                  <input
                    type="file"
                    accept=".csv"
                    required
                  ...
                  <label htmlFor="csv-file-input" className="cursor-pointer space-y-2 block">
                    ...
                    <div className="text-xs text-gray-600 font-medium">
                      {csvFile ? (
                        <span className="text-brand-navy font-bold">{csvFile.name}</span>
                      ) : (
                        <span>Click to browse or upload CSV file</span>
                      )}
                    </div>
                    <p className="text-[10px] text-gray-400">Max size 5MB (comma-separated values)</p>
                  </label>
                </div>
                ...
                <div className="flex gap-2.5 pt-2 border-t border-gray-100">
                  <button
                    type="button"
                    onClick={() => { setShowImportModal(false); setCsvFile(null); }}
                  ...
                  <button
                    ...
                    className="flex-1 py-2 rounded-xl bg-brand-navy text-white text-xs font-bold hover:bg-brand-navy-light disabled:opacity-50 transition-colors"
                  >
                    {isImporting ? 'Importing contacts...' : 'Process CSV'}
                  </button>
                </div>
              </form>
            )}
