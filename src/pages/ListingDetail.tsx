...
export function ListingDetailPage() {
  ...
  return (
    <Layout title={listing.address}>
      ...
      {/* Showing modal */}
      {showShowingModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setShowShowingModal(false)}>
          <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-md mx-4 space-y-4" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-lg font-bold text-gray-800">Log Showing</h3>
            <div>
              <label className="mb-1 block text-xs font-semibold uppercase tracking-wide text-gray-500">Buyer Name</label>
              <input
            ...
            <div>
              <label className="mb-1 block text-xs font-semibold uppercase tracking-wide text-gray-500">Date & Time</label>
              <input
                type="datetime-local"
            ...
            <div>
              <label className="mb-1 block text-xs font-semibold uppercase tracking-wide text-gray-500">Feedback</label>
              <textarea
            ...
            <div className="flex gap-2 pt-2">
              ...
              <button
                disabled={submittingShowing || !showingForm.buyerName.trim() || !showingForm.shownAt}
                onClick={async () => {
                  ...
                  try {
                    const res = await fetch('/api/showings', {
                      ...
                      body: JSON.stringify({
                        ...
                        shownAt: new Date(showingForm.shownAt).toISOString(),
                        feedback: showingForm.feedback.trim() || undefined,
                    ...
                    alert('Showing logged successfully')
                  } catch (e: any) {
                    alert(e.message)
                  } finally {
              ...
              >
                {submittingShowing ? 'Saving...' : 'Save Showing'}
              </button>
            </div>
          </div>
        </div>
