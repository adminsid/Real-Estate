  ...
  customModules: [
    {
      ...
      externalUrl: 'https://openhouse.primeamericarealestate.com',
      status: 'active',
      category: 'marketing',
      color: '#EC4899',
      adminOnly: true
...
export function DashboardPage() {
  ...
  return (
    <Layout title="Dashboard">
      {/* ── Welcome banner & Action Toolbar ──────────────────────────── */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          <p className="text-gray-500 text-sm">
            {greeting()}{user ? `, ${user.name}` : ''}
          </p>
          <h2 className="text-xl font-bold text-gray-800">Brokerage Dashboard</h2>
        </div>
        {isAdmin && (
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4 border-b border-gray-100 pb-4">
          <div>
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-xl font-bold text-gray-900">{greeting()}{user ? `, ${user.name.split(' ')[0]}` : ''}</h2>
            </div>
            <p className="text-xs text-gray-500 mt-1">Daily Brokerage Briefing • {new Date().toLocaleDateString('en-US', { weekday: 'long', month: 'short', day: 'numeric', year: 'numeric' })}</p>
          </div>
          <div className="flex items-center gap-2">
            <Link to="/transactions/pipeline" className="text-xs font-bold text-blue-600 hover:text-blue-800 bg-blue-50 px-3 py-1.5 rounded-lg border border-blue-100 flex items-center gap-1">
              View Deal Pipeline <Icons.ArrowRight className="h-3.5 w-3.5" />
            </Link>
          </div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Link to="/transactions/pipeline" className="rounded-xl border border-gray-100 bg-slate-50/60 hover:bg-slate-100/80 p-4 transition-colors">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs font-bold text-gray-600 uppercase tracking-wide">Deals Closing Soon</span>
              <Icons.Calendar className="h-4 w-4 text-blue-600" />
            </div>
            <p className="text-2xl font-bold text-gray-900">{upcomingDeals.length}</p>
            <p className="text-xs text-gray-500 mt-0.5">{upcomingDeals.length > 0 ? `${upcomingDeals.length} deal(s) approaching close date` : 'No upcoming close dates this week'}</p>
          </Link>
          <Link to="/transactions/pipeline" className="rounded-xl border border-gray-100 bg-slate-50/60 hover:bg-slate-100/80 p-4 transition-colors">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs font-bold text-gray-600 uppercase tracking-wide">Active Deals Tracked</span>
              <Icons.CheckCircle2 className="h-4 w-4 text-emerald-600" />
            </div>
            <p className="text-2xl font-bold text-gray-900">{stats.company.transactions}</p>
            <p className="text-xs text-gray-500 mt-0.5">{stats.company.transactions > 0 ? `${stats.company.transactions} active deal workflows` : 'No active deal workflows'}</p>
          </Link>
          <Link to="/inventory" className="rounded-xl border border-gray-100 bg-slate-50/60 hover:bg-slate-100/80 p-4 transition-colors">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs font-bold text-gray-600 uppercase tracking-wide">Active Listings</span>
              <Icons.Home className="h-4 w-4 text-amber-600" />
            </div>
            <p className="text-2xl font-bold text-gray-900">{stats.company.activeListings}</p>
            <p className="text-xs text-gray-500 mt-0.5">{stats.company.activeListings > 0 ? `${stats.company.activeListings} listings live in inventory` : 'No live listings'}</p>
          </Link>
        </div>
      </div>
